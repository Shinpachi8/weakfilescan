...
class FuzzEnginer(object):
    ...
    def __init__(self, urls, refer_to_val=0, fuzz_type='dir'):
    ...
    class FuzzWorker(threading.Thread):
        ...
        def __init__(self, queue, fuzz_type):
            ...
            while True:
                ...
                try:  # 用hack方法，no_timeout读取Queue队列，直接异常退出线程避免阻塞
                    ...
                    if results.status_code in exclude_status:
                        ...
                        # 加入是否为备份文件结尾判断
                        if (self.fuzz_type == 'file') and backup_file(url):
                            content_type = results.headers.get(
                                'Content-Type', '')
                            if content_type == '' or content_type.find('text') >= 0 or content_type.find('html') or content_type.find('json') >= 0:
                                continue
                            else:
                        ...
                        elif (self.fuzz_type == 'file') and (not check_404_page(url)):
                            resources[results.status_code][url] = {
                                'is_redirect': is_redirect,
                                'history': results.history,
                                'request': results.url
                        ...
                        # 存在content-length属性
                        elif (self.fuzz_type == 'dir') and results.headers.get('content-length'):
                            is_redirect = True if len(
                                results.history) > 0 else False
                            if not is_redirect:  # 未发生url跳转
                                # 如果返回了content-length属性，同时大小<100KB，加入404错误定义检测
                                # 1000 = 1k, 100kb = 100000
                        ...
                        elif (self.fuzz_type == 'dir') and refer_to_val >= 50:  # print '404错误回显可工作'
                            regex = re.compile(page_not_found_reg)
                            # print '找到错误定义，成功返回404信息'
                            if not regex.findall(results.text):
                                is_redirect = True if len(
    ...
    def start(self):
        ...
        for i in xrange(threads_count):
            threads.append(self.FuzzWorker(queue, self.fuzz_type))
