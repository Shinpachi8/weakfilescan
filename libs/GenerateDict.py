...
"""
    ...
    fuzz_filename = ProcessDic('./dict/filename.lst',
                               fuzz_filename_replace).parser()
"""
    ...
    def parser(self):
        ...
        if len(self.replace_dict) >= 1:
            for key in self.replace_dict.keys():
                ...
                #     new_replace_vale = [i.replace('\.', '.')
                #                         for i in replace_value.split('|')]
                #     for replace_value in new_replace_vale:
...
if __name__ == '__main__':
    fuzz_filename_replace = {
        '%EXT%': 'aspx', '%BAK_EXT%': '\.rar|\.zip|\.gz|\.tar|\.tgz|\.tar\.gz|\.7z|\.z|\.bz2|\.tar\.bz2|\.iso|\.cab'}
    fuzz_filename = ProcessDic(
        '../dict/test.lst', fuzz_filename_replace).parser()
    print(fuzz_filename)
