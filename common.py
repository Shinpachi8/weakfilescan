...
def check_404_page(url):
    ...
    page_of_404 = url[:(last_index + 1)] + '/bbscan-404-page-check'
    page_of_404_result = http_request_get(page_of_404)
    original_result = http_request_get(url)

    regex = re.compile(page_not_found_reg)
    if regex.findall(original_result.text):
        is_404 = True
    elif len(original_result.text.strip()) < 15:
        is_404 = True
    elif page_of_404_result.status_code != original_result.status_code:
        is_404 = False
    elif (page_of_404_result.status_code == original_result.status_code) and (len(page_of_404_result.text) == len(original_result.text)):
        is_404 = True
    elif (page_of_404_result.status_code == original_result.status_code) and (len(page_of_404_result.text) != len(original_result.text)):
        _len = len(original_result.text)
        _min = min(_len, page_of_404_result.text)
        if _min == 0:
            _min = 10.0
        if float(abs(_len - page_of_404_result.text)) / _min > 0.3:
            is_404 = True
    if is_404 is False:
        print "[%s] %s => %s" % (url, 'is_404', str(is_404))
    return is_404
